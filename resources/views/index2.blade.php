...
<head>
    ...
    <style>
        #togglePassword svg { color: #e2e8f0; }
        body.light-mode #togglePassword svg { color: #1e293b; }
        .modal {
        ...
        .modal-content {
            background-color: #1e293b; margin: auto; padding: 0;
            border-radius: 0.75rem; width: 95%; max-width: 950px;
            max-height: 90vh; overflow-y: auto; overflow-x: hidden;
            box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.5);
        }
        .modal-content::-webkit-scrollbar { width: 8px; }
        .modal-content::-webkit-scrollbar-track { background: #1e293b; border-radius: 0 0.75rem 0.75rem 0; }
        ...
        .modal-content iframe { width: 100%; height: auto; min-height: 600px; border: none; display: block; }
        .light-mode .modal-content { background-color: #ffffff; }
        .light-mode .modal-content::-webkit-scrollbar-track { background: #ffffff; }
        ...
        .light-mode .modal-header { background-color: #f1f5f9 !important; }
        .light-mode .modal-content .modal-title { color: #1f2937 !important; }
        .light-mode .modal-content .modal-close { color: #1f2937 !important; }
        .light-mode .modal-content .modal-label { color: #6b7280 !important; }
        .light-mode .modal-content .modal-value { color: #1f2937 !important; }
        .light-mode .modal-content .modal-border { border-color: #e5e7eb !important; }
        .light-mode .modal-content .modal-btn { background-color: #e5e7eb !important; color: #1f2937 !important; }
        .light-mode .modal-content .modal-payment-text { color: #1f2937 !important; }
    </style>
</head>
<body class="bg-slate-900 transition-colors duration-300">
        ...
        <div class="bg-slate-800 rounded-lg shadow-lg p-8 main-card">
            <form method="POST" action="{{ route('tagihan.cek2') }}" id="billForm">
                ...
                <div class="mb-6 relative">
                    ...
                    <div class="relative">
                        <input type="password" name="password" id="password" placeholder="Masukkan password" class="w-full px-4 py-3 pr-12 rounded-lg border bg-slate-700 text-white border-slate-600 focus:outline-none focus:ring-2 focus:ring-blue-500 input-field" required/>
                        <button type="button" id="togglePassword" class="absolute inset-y-0 right-2 flex items-center px-2 text-slate-200 hover:text-white focus:outline-none">
                            <svg id="iconEye" xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                                <path stroke-linecap="round" stroke-linejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.477 0 8.268 2.943 9.542 7-1.274 4.057-5.065 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                            </svg>
                            <svg id="iconEyeOff" xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 hidden" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.477 0-8.268-2.943-9.542-7a10.05 10.05 0 012.223-3.444M6.18 6.18A9.966 9.966 0 0112 5c4.477 0 8.268 2.943 9.542 7a9.974 9.974 0 01-4.245 5.253M3 3l18 18"/>
                            </svg>
                        </button>
                    </div>
                </div>
                <div class="mb-6">
                    <label class="block text-white label-text mb-2">Tahun Akademik <span class="text-red-500">*</span></label>
                    <select id="academic_year" name="academic_year" class="w-full px-4 py-3 rounded-lg border bg-slate-700 text-white border-slate-600 focus:outline-none focus:ring-2 focus:ring-blue-500 input-field">
            ...
            <div class="mt-8 pt-8 border-t border-slate-600 border-section">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <a href="/Gambar_Panduan_Bayar.jpeg" target="_blank" class="bg-purple-600 hover:bg-purple-700 text-white py-4 px-6 rounded-lg font-semibold transition-colors flex items-center justify-center gap-2 shadow-sm">
                    ...
                    <a href="/Booklet_Panduan_Bayar.pdf" target="_blank" class="bg-red-600 hover:bg-red-700 text-white py-4 px-6 rounded-lg font-semibold transition-colors flex items-center justify-center gap-2 shadow-sm">
                        <svg class="w-5 h-5 opacity-90" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <table class="w-full border-collapse">
                                <thead>
                                    <tr class="bg-slate-700 table-header">
                                        ...
                                        <th class="border border-slate-600 table-border px-4 py-3 text-left text-white main-text">URUTAN</th>
                                        ...
                                        <th class="border border-slate-600 table-border px-4 py-3 text-left text-white main-text">TANGGAL BUAT TAGIHAN</th>
                                ...
                                <tbody id="tagihanTableBody">
                                    @forelse($result['data']['tagihan'] as $i => $tagih)
                                        <tr data-index="{{ $i }}">
                                            ...
                                            <td class="border border-slate-600 table-border px-4 py-3 text-white table-text">{{ $tagih['FURUTAN'] ?? '-' }}</td>
                                            ...
                                            <td class="border border-slate-600 table-border px-4 py-3 text-white table-text">
                                                {{ $tagih['FTGLTagihan'] ? \Carbon\Carbon::parse($tagih['FTGLTagihan'])->format('Y-m-d') : '-' }}
                                            </td>
                                            <td class="border border-slate-600 table-border px-4 py-3 text-center">
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="8" class="border border-slate-600 table-border px-4 py-8 text-center text-gray-400 secondary-text">Tidak ada data tersedia</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                            </table>
                        ...
                        <div class="overflow-x-auto">
                            <table class="w-full border-collapse">
                                ...
                                <tbody id="lunasTableBody">
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="6" class="border border-slate-600 table-border px-4 py-8 text-center text-gray-400 secondary-text">Tidak ada tagihan lunas</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                            </table>
    ...
    <div id="detailModal" class="modal">
        <div class="modal-content">
            ...
            <div class="p-6">
                ...
                <div id="modalDetailTable" class="overflow-x-auto"></div>
    ...
    <div id="paymentModal" class="modal">
        <div class="modal-content">
            ...
            <div class="p-6">
                <div id="paymentContent" class="modal-payment-content"></div>
            </div>
        </div>
    </div>
    ...
    // Selalu bersihkan dulu widget sebelum render baru (PENTING!)
    widget.innerHTML = '';
    turnstileToken = null;
    ...
    turnstile.render('#turnstile-widget', {
        ...
        'error-callback': function(errorCode) {
            console.log('Turnstile error code:', errorCode);  // lihat di console F12
            turnstileToken = null;
            document.getElementById('cfTurnstileResponse').value = '';
            // JANGAN langsung alert, biar tidak ganggu user
            // alert('Verifikasi gagal! Silakan refresh halaman.');  ← comment dulu
        },
        ...
        function paginateTagihan(allRows) {
            ...
            if (tagihanShowAll) {
                ...
                for (let i = startIndex; i < endIndex; i++) allRows[i].style.display = '';
            }
            ...
            if (infoEl) infoEl.textContent = `Menampilkan ${startItem} - ${endItem} dari ${totalRows} data`;
            renderTagihanPagination(tagihanShowAll ? 1 : Math.ceil(totalRows / tagihanPerPage), totalRows);
        }
        ...
        function paginateLunas(allRows) {
            ...
            if (infoEl) infoEl.textContent = `Menampilkan ${startItem} - ${endItem} dari ${totalRows} data`;
            renderLunasPagination(lunasShowAll ? 1 : Math.ceil(totalRows / lunasPerPage), totalRows);
        }

        function renderTagihanPagination(totalPages, totalRows) {
            const paginationEl = document.getElementById('tagihanPagination');
            if (!paginationEl) return;
            if (tagihanShowAll || totalRows === 0) { paginationEl.innerHTML = ''; return; }
            let html = '';
            if (tagihanCurrentPage > 1) html += `<button onclick="goToTagihanPage(${tagihanCurrentPage - 1})" class="px-3 py-2 rounded-lg bg-slate-700 text-white hover:bg-slate-600 transition-colors text-sm">Prev</button>`;
            const maxButtons = 5;
            ...
            if (startPage > 1) {
                ...
                if (startPage > 2) html += `<span class="px-3 py-2 text-white">...</span>`;
            }
            ...
            if (endPage < totalPages) {
                if (endPage < totalPages - 1) html += `<span class="px-3 py-2 text-white">...</span>`;
            ...
            if (tagihanCurrentPage < totalPages) html += `<button onclick="goToTagihanPage(${tagihanCurrentPage + 1})" class="px-3 py-2 rounded-lg bg-slate-700 text-white hover:bg-slate-600 transition-colors text-sm">Next</button>`;
        ...
        function renderLunasPagination(totalPages, totalRows) {
            ...
            if (!paginationEl) return;
            if (lunasShowAll || totalRows === 0) { paginationEl.innerHTML = ''; return; }
            let html = '';
            if (lunasCurrentPage > 1) html += `<button onclick="goToLunasPage(${lunasCurrentPage - 1})" class="px-3 py-2 rounded-lg bg-slate-700 text-white hover:bg-slate-600 transition-colors text-sm">Prev</button>`;
            const maxButtons = 5;
            ...
            if (lunasCurrentPage < totalPages) html += `<button onclick="goToLunasPage(${lunasCurrentPage + 1})" class="px-3 py-2 rounded-lg bg-slate-700 text-white hover:bg-slate-600 transition-colors text-sm">Next</button>`;
            paginationEl.innerHTML = html;
        }

        function goToTagihanPage(page) {
            tagihanCurrentPage = page;
            const allRows = Array.from(document.getElementById('tagihanTableBody').querySelectorAll('tr[data-index]'));
            paginateTagihan(allRows);
        }

        function goToLunasPage(page) {
            lunasCurrentPage = page;
            const allRows = Array.from(document.getElementById('lunasTableBody').querySelectorAll('tr[data-index]'));
            paginateLunas(allRows);
        }

        function changeTagihanPerPage() {
            tagihanPerPage = parseInt(document.getElementById('tagihanPerPage').value);
            tagihanCurrentPage = 1;
            tagihanShowAll = false;
            paginateTagihan(Array.from(document.getElementById('tagihanTableBody').querySelectorAll('tr[data-index]')));
        }

        function changeLunasPerPage() {
            lunasPerPage = parseInt(document.getElementById('lunasPerPage').value);
            lunasCurrentPage = 1;
            lunasShowAll = false;
            paginateLunas(Array.from(document.getElementById('lunasTableBody').querySelectorAll('tr[data-index]')));
        }
        ...
        function toggleTheme() {
            ...
            if (currentTheme === 'dark') {
                ...
                document.querySelectorAll('.main-card, .result-card').forEach(el => { el.classList.remove('bg-slate-800'); el.classList.add('bg-white'); });
                document.querySelectorAll('.input-field').forEach(el => { el.classList.remove('bg-slate-700', 'text-white', 'border-slate-600'); el.classList.add('bg-white', 'text-gray-900', 'border-gray-300'); });
                document.querySelectorAll('.label-text').forEach(el => { el.classList.remove('text-white'); el.classList.add('text-gray-900'); });
                document.querySelectorAll('.main-text').forEach(el => { el.classList.remove('text-white'); el.classList.add('text-gray-900'); });
                document.querySelectorAll('.secondary-text').forEach(el => { el.classList.remove('text-gray-400'); el.classList.add('text-gray-600'); });
                document.querySelectorAll('.border-section').forEach(el => { el.classList.remove('border-slate-600'); el.classList.add('border-gray-300'); });
                document.querySelectorAll('.table-header').forEach(el => { el.classList.remove('bg-slate-700'); el.classList.add('bg-gray-200'); });
                document.querySelectorAll('.table-border').forEach(el => { el.classList.remove('border-slate-600'); el.classList.add('border-gray-300'); });
                document.querySelectorAll('.table-text').forEach(el => { el.classList.remove('text-white'); el.classList.add('text-gray-900'); });
            } else {
                currentTheme = 'dark';
                ...
                document.querySelectorAll('.main-card, .result-card').forEach(el => { el.classList.remove('bg-white'); el.classList.add('bg-slate-800'); });
                document.querySelectorAll('.input-field').forEach(el => { el.classList.remove('bg-white', 'text-gray-900', 'border-gray-300'); el.classList.add('bg-slate-700', 'text-white', 'border-slate-600'); });
                document.querySelectorAll('.label-text').forEach(el => { el.classList.remove('text-gray-900'); el.classList.add('text-white'); });
                document.querySelectorAll('.main-text').forEach(el => { el.classList.remove('text-gray-900'); el.classList.add('text-white'); });
                document.querySelectorAll('.secondary-text').forEach(el => { el.classList.remove('text-gray-600'); el.classList.add('text-gray-400'); });
                document.querySelectorAll('.border-section').forEach(el => { el.classList.remove('border-gray-300'); el.classList.add('border-slate-600'); });
                document.querySelectorAll('.table-header').forEach(el => { el.classList.remove('bg-gray-200'); el.classList.add('bg-slate-700'); });
                document.querySelectorAll('.table-border').forEach(el => { el.classList.remove('border-gray-300'); el.classList.add('border-slate-600'); });
                document.querySelectorAll('.table-text').forEach(el => { el.classList.remove('text-gray-900'); el.classList.add('text-white'); });
            }
            saveThemePreference();
            resetTurnstile();
        }
        ...
        function toggleSelectAll(checkbox) {
            const tbody = document.getElementById('tagihanTableBody');
            Array.from(tbody.querySelectorAll('tr[data-index]'))
                .filter(row => row.style.display !== 'none')
                .forEach(row => { const cb = row.querySelector('.tagihan-checkbox'); if (cb) cb.checked = checkbox.checked; });
        }
        ...
        function showDetailModal(tagihan) {
            ...
            document.getElementById('modalNamaTagihan').textContent = formattedName;
            document.getElementById('modalTahunAkademik').textContent = tagihan.tahun_akademik_tagihan || '-';
            let tableHTML = '<table class="w-full border-collapse mt-4"><thead><tr class="modal-table-header">';
            ...
            if (Array.isArray(tagihan.detail) && tagihan.detail.length > 0) {
                tagihan.detail.forEach(d => {
                    tableHTML += `<tr><td class="border modal-table-border px-4 py-2 modal-table-cell">${d.akun_detail || '-'}</td>`;
                    tableHTML += `<td class="border modal-table-border px-4 py-2 modal-table-cell">Rp. ${parseInt(d.nominal_detail || 0).toLocaleString('id-ID')}</td></tr>`;
                });
            } else {
            ...
            document.getElementById('modalDetailTable').innerHTML = tableHTML;
        ...
        function showPaymentModal() {
            ...
            if (checkboxes.length === 0) { alert('Silakan pilih minimal satu tagihan untuk dibayar!'); return; }
            const selectedTagihan = [];
            const allTagihanData = @json($result['data']['tagihan'] ?? []);
            checkboxes.forEach(checkbox => {
                const tagihanId = checkbox.value;
                const fullData = allTagihanData.find((item, idx) => (item.id && item.id == tagihanId) || idx == tagihanId);
                if (fullData) {
            ...
            document.querySelectorAll('.modal-payment-text').forEach(el => {
                el.classList.add(currentTheme === 'light' ? 'text-gray-900' : 'text-white');
            });
            modal.classList.add('active');
            setTimeout(() => {
        ...
        document.getElementById('billForm').addEventListener('submit', function(e) {
            if (!turnstileToken) { e.preventDefault(); alert('Silakan selesaikan verifikasi keamanan terlebih dahulu!'); return false; }
        });
